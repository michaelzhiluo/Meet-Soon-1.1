...
- (id)initWithFrame:(CGRect)frame logic:(CalendarMonthLogic *)aLogic {
    ...
    /**calculate numberOfWeeks start**/
    int countOfdaysInOtherMonthInLastMonth=0;
    for (NSInteger aWeek = 0; aWeek <= numberOfWeeks; aWeek ++) {
        
        for (NSInteger aWeekday = 1; aWeekday <= numberOfDaysInWeek; aWeekday ++) {
            NSDate *dayDate = [CalendarMonthLogic dateForWeekday:aWeekday 
                                                     onWeek:aWeek 
                                              referenceDate:[aLogic referenceDate]];
            if ([aLogic distanceOfDateFromCurrentMonth:dayDate] > 0) {
                countOfdaysInOtherMonthInLastMonth = countOfdaysInOtherMonthInLastMonth+1;
            }
        }
    }
    if(countOfdaysInOtherMonthInLastMonth >= 7){
        numberOfWeeks = 4;
    }else{
        numberOfWeeks = 5;
    }
    /**calculate numberOfWeeks end**/
    ...
    if ((self = [super initWithFrame:frame])) {
		...
		aLabel.font = [UIFont boldSystemFontOfSize:23];
		...
		// Setup weekday names
        //set up week display(firstWeekday means  order by weekday)
		...
		for (NSInteger aWeek = 0; aWeek <= numberOfWeeks; aWeek ++) {
			...
			for (NSInteger aWeekday = 1; aWeekday <= numberOfDaysInWeek; aWeekday ++) {
				...
				if ([aLogic distanceOfDateFromCurrentMonth:dayDate] != 0) {
                    titleColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"CalendarTitleDimColorFont.png"]];
				}
				...
				dayButton.titleLabel.font = [UIFont boldSystemFontOfSize:23];
...
- (void)selectButtonForDate:(NSDate *)aDate {
	...
	if (aDate != nil) {
		...
		NSDate *todayDate = [CalendarMonthLogic dateForToday];
        if(selectedButton > 34 && [buttonsIndex count] <= 35) return;
		UIButton *button = [buttonsIndex objectAtIndex:selectedButton];
